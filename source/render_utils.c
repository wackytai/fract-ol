...
/*   Updated: 2023/05/05 11:28:51 by tlemos-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_zoom(t_data *data, int x, int y)
{
	if (x <= W_WIDTH && y <= W_HEIGHT)
	{
		if (data->f->z_flag > 0)
			...
			data->f->zoom *= 0.85;
		data->f->offset.x += (((double)x - (W_WIDTH / 2)) / W_WIDTH) * data->f->zoom;
		data->f->offset.y += (((double)y - (W_HEIGHT / 2)) / W_HEIGHT) * data->f->zoom;
		update_screen(data);
	}
	return ;
}
